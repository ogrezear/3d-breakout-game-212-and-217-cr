#include "main.h"
//This pice of code has been taken from http://www.opengl.org/discussion_boards/ubbthreads.php?ubb=showflat&Number=267363
//The texture loading program I am using has been developed by someone else and I do not intend to 
//present it as being my work
int LoadBitmap(const char *filename)
{
    ...
    fread(&infoheader.biWidth, sizeof(int), 1, file); //Read the file width

    fread(&infoheader.biHeight, sizeof(int), 1, file);//Read file Height
    ...
    // read the bpp
    fread(&infoheader.biBitCount, sizeof(unsigned short int), 1, file); // Check the depth of the bmp to be 24
